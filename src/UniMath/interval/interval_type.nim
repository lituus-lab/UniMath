...
func certaintySign*(i: Interval[float32]): int {.inline.} =
  ## `float32` counterpart, so the pair stays symmetric with `isUncertain`.
  if i.upper < 0.0f: return -1
  if i.lower > 0.0f: return 1
  return 0
