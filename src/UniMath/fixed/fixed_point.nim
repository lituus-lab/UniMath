...
template verifyFloatFracBits*(FracBits: static[int]) =
  ## Extra guard for the float64 conversion paths. `2^FracBits` is the scale
  ## factor there, and it is `+inf` from 1024 up: `Fixed[BigInt, 1024]` built
  ## from 1.0 would store `inf` and read back as 0.0, losing the value in
  ## silence. Integer construction is unaffected and stays available.
  static: doAssert FracBits <= 1023,
    "float64 conversion needs FracBits <= 1023; 2^1024 is +inf"
...
func toFixed*[T; FracBits: static[int]](val: SomeFloat): Fixed[T, FracBits] =
  ...
  verifyFracBits(T, FracBits)
  verifyFloatFracBits(FracBits)
  result.data = storeFloat(val, T, FracBits)

template verifySplitFracBits*(T: typedesc; FracBits: static[int]) =
  ## `toInt`/`fracPart` shift and mask inside `T`. A `FracBits` at or above the
  ## storage width makes both undefined -- `Fixed[int16, 20]`, which the LUT
  ## path does instantiate, would build its mask as `int16(1) shl 20`. The type
  ## itself stays constructible; only this split is refused.
  when T is SomeInteger:
    static: doAssert FracBits < sizeof(T) * 8,
      "toInt/fracPart need FracBits below the storage width"

func toInt*[T; FracBits: static[int]](a: Fixed[T, FracBits]): T {.inline.} =
  ## Integer part, as the floor: an arithmetic shift rounds down, so
  ## `toInt(-3.75) == -4`, not -3. Paired with `fracPart`'s non-negative
  ## residue this splits any value as `toInt(a) + fracPart(a) == a`, for
  ## negatives too -- `-4 + 0.25 == -3.75`. Nim's `shr` on a signed integer is
  ## already the arithmetic shift this needs.
  verifySplitFracBits(T, FracBits)
  a.data shr FracBits

func fracPart*[T; FracBits: static[int]](a: Fixed[T, FracBits]): T {.inline.} =
  ## Fractional bits, always non-negative -- the residue above `toInt`'s floor.
  verifySplitFracBits(T, FracBits)
  let one = T(1)
