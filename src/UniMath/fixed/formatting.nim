...
func toFloat*[T; FracBits: static[int]](a: Fixed[T, FracBits]): float =
  ...
  ## `FracBits >= 64` (e.g. `Fixed[BigInt, 100]`), dividing by 0.0. Above 1023
  ## the scale is `+inf` and the value would divide away to 0.0, so the same
  ## guard as `toFixed(SomeFloat)` applies.
