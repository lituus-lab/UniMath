...
func sqrtNewtonGeneric*[F: OrderedField](n: F,
  ...
  ##
  ## `iterations` is a hard cap, not a convergence guarantee, and the seed is
  ## `n` itself. Newton halves the ratio `x / sqrt(n)` per step while that
  ## ratio is large, so a huge argument spends the whole budget in the linear
  ## approach phase: `sqrtNewtonGeneric(1e300)` needs on the order of 500
  ## steps, far past the default. Conversely a bounded exact backend cannot
  ## afford the default at all -- each step squares numerator and denominator,
  ## so `Rational[int64]` overflows within a handful of iterations (see
  ## `rational_math.sqrt`). Size `iterations` to the backend and the argument;
  ## the early exit returns as soon as the iterate stabilises.
  body:
    let z0 = zero(F)
    if n < z0:
