...
func gammaLanczos*[F: Field](
  ...
  ##
  ## The pole test is `sinFunc(piConst * x) == zero`, so it is only exact when
  ## the witnesses are: with an approximate `piConst` or `sinFunc` a
  ## non-positive integer argument yields a tiny non-zero sine and returns a
  ## large finite value instead of raising. The `ValueError` is guaranteed for
  ## the float64 instantiation, not for arbitrary witnesses.
  body:
    let z0 = zero(F)
    let o1 = one(F)
