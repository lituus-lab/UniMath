...
suite "BigFloat vs MPFR — arithmetic":
  test "add/sub/mul/div match the correctly-rounded float64":
    ...
    for (op, a, b) in cases:
      # 128, not 64: the check reduces the candidate with toFloat64 and then
      # demands bit equality with the correctly-rounded binary64 reference, so
      # a 64-bit working precision rounds twice (106 exact bits for a product,
      # then 64, then 53). oracle.nim documents prec >= 107 for exactly this.
      let ba = initBigFloat(a, 128)
      let bb = initBigFloat(b, 128)
      let cand =
        case op
