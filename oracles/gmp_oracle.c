...
int main(void) {
  ...
  while ((len = getline(&line, &cap, stdin)) != -1) {
    ...
    if (strcmp(op, "add") == 0 || strcmp(op, "sub") == 0 ||
      ...
      mp_set(a, sa); mp_set(b, sb);
      /* mpz_cmp only guarantees the sign; the protocol says -1, 0 or 1. */
      { int c = mpz_cmp(a, b); printf("%d\n", (c > 0) - (c < 0)); }
      ...
      /* mp_set, not a bare mpz_set_str: the return value is the only signal
       * that a token was malformed, and ignoring it leaves the operand at its
       * previous value, so the oracle reports a wrong reference instead of
       * failing. A zero denominator is worse -- mpq_canonicalize is undefined
       * for it. */
      mp_set(mpq_numref(pa), na);
      mp_set(mpq_denref(pa), da);
      mp_set(mpq_numref(pb), nb);
      mp_set(mpq_denref(pb), db);
      if (mpz_sgn(mpq_denref(pa)) == 0 || mpz_sgn(mpq_denref(pb)) == 0) {
        fprintf(stderr, "%s: zero denominator\n", op); exit(1);
      }
      mpq_canonicalize(pa); mpq_canonicalize(pb);
      { int c = mpq_cmp(pa, pb); printf("%d\n", (c > 0) - (c < 0)); }
