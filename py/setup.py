...
"""Build unimath._core, a Cython extension over the UniMath C ABI.
...
PKG_DIR = os.path.join(HERE, "unimath")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniMath.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniMath.lib", False
    RUNTIME_DIRS, LINK_ARGS, NIMBLE_TASK = [], [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniMath.dylib", True
    RUNTIME_DIRS, LINK_ARGS, NIMBLE_TASK = [], ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniMath.so", True
...
def nim_project_dir():
    """Where UniMath.nimble lives: the real repo root in a normal checkout,
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unimath from "
...
# The sdist ships the pre-transpiled unimath/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unimath", "_core.pyx")
ext = Extension(
    "unimath._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unimath", "_core.c")],
...
setup(
    ...
    package_data={"unimath": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unimath": ["_core.c"]},
    zip_safe=False,
)